...
export default function NavItem({ item, level, isParents = false, setSelectedID }) {
  ...
  const iconColor = item?.color || (isSelected ? theme.palette.secondary.main : theme.palette.text.primary);
  const itemIcon = item?.icon ? (
    <Icon stroke={1.5} size={drawerOpen ? '20px' : '24px'} style={{ color: iconColor, ...(isParents && { fontSize: 20, stroke: '1.5' }) }} />
  ) : (
    <FiberManualRecordIcon sx={{ width: isSelected ? 8 : 6, height: isSelected ? 8 : 6, color: iconColor }} fontSize={level > 0 ? 'inherit' : 'medium'} />
  );
